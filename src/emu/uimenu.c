...
static void select_game_render(const menu_extra *extra, float origx1, float origy1, float origx2, float origy2)
{
	...
	else
	{
		...
		sprintf(&tempbuf[0][0], "M.A.M.E. %s", build_version);

		/* next lines are copyright */
		sprintf(&tempbuf[1][0], "Copyright 2007, Nicola Salmoria");
		sprintf(&tempbuf[2][0], "and the MAME team");

		/* next line is website */
		sprintf(&tempbuf[3][0], "http://mamedev.org");
	}
	...
	ui_draw_text_full(&tempbuf[1][0], x1, y1, x2 - x1, JUSTIFY_CENTER, WRAP_TRUNCATE,
					  DRAW_NORMAL, ARGB_WHITE, ARGB_BLACK, NULL, NULL);
	y1 += ui_get_line_height();

	ui_draw_text_full(&tempbuf[2][0], x1, y1, x2 - x1, JUSTIFY_CENTER, WRAP_TRUNCATE,
					  DRAW_NORMAL, ARGB_WHITE, ARGB_BLACK, NULL, NULL);
	y1 += ui_get_line_height();

	ui_draw_text_full(&tempbuf[3][0], x1, y1, x2 - x1, JUSTIFY_CENTER, WRAP_TRUNCATE,
					  DRAW_NORMAL, ARGB_WHITE, ARGB_BLACK, NULL, NULL);
	y1 += ui_get_line_height();
}
