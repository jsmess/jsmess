...
{
	work_unit_shared	shared;					/* shared data */
	quad_extent			extent[SCANLINES_PER_BUCKET]; /* array of scanline extents */
};
...
struct _polygon_info
{
	...
	poly_draw_tri_scanline tricallback;			/* callback to handle a scanline's worth of work */
	poly_draw_quad_scanline quadcallback;		/* callback to handle a scanline's worth of work */
...
UINT32 poly_render_triangle(poly_manager *poly, void *dest, const rectangle *cliprect, poly_draw_tri_scanline callback, int paramcount, const poly_vertex *v1, const poly_vertex *v2, const poly_vertex *v3)
{
	...
	polygon->poly = poly;
	polygon->dest = dest;
	polygon->tricallback = callback;
	polygon->quadcallback = NULL;
...
UINT32 poly_render_triangle_custom(poly_manager *poly, void *dest, const rectangle *cliprect, poly_draw_tri_scanline callback, int startscanline, int numscanlines, const tri_extent *extents)
{
	...
	for (curscan = v1yclip; curscan < v3yclip; curscan += scaninc)
	{
		...
		for (extnum = 0; extnum < unit->shared.count_next; extnum++)
		{
			const tri_extent *extent = &extents[(curscan + extnum) - startscanline];
...
UINT32 poly_render_quad(poly_manager *poly, void *dest, const rectangle *cliprect, poly_draw_quad_scanline callback, int paramcount, const poly_vertex *v1, const poly_vertex *v2, const poly_vertex *v3, const poly_vertex *v4)
{
	...
	polygon->poly = poly;
	polygon->dest = dest;
	polygon->tricallback = NULL;
	polygon->quadcallback = callback;
	polygon->extra = poly->extra[poly->extra_next - 1];
...
static void *poly_item_callback(void *param, int threadid)
{
	while (1)
	{
		...
		for (curscan = 0; curscan < count; curscan++)
			if (polygon->tricallback != NULL)
				(*polygon->tricallback)(polygon->dest, unit->shared.scanline + curscan, &unit->tri.extent[curscan], &polygon->params, polygon->extra, threadid);
			else
				(*polygon->quadcallback)(polygon->dest, unit->shared.scanline + curscan, &unit->quad.extent[curscan], polygon->extra, threadid);
