...
{
	UINT16 data = memory_read_byte( cpustate->program, 0x1c) << 8;
	cpustate->SP = memory_read_byte( cpustate->program, 0x1d);
...
INLINE void sm8500_do_interrupt(sm8500_state *cpustate, UINT16 vector) {
	...
	cpustate->SYS = memory_read_byte(cpustate->program, 0x19);	
	cpustate->PS1 = memory_read_byte(cpustate->program, 0x1f);	
	/* Push PC */
	...
	if ( cpustate->CheckInterrupts ) {
		...
		while( irqline < 11 ) {
			if ( cpustate->IFLAGS & ( 1 << irqline ) ) {
				cpustate->halted = 0;
				cpustate->IE0 = memory_read_byte(cpustate->program, 0x10);
				cpustate->IE1 = memory_read_byte(cpustate->program, 0x11);
				cpustate->IR0 = memory_read_byte(cpustate->program, 0x12);
				cpustate->IR1 = memory_read_byte(cpustate->program, 0x13);
				cpustate->PS0 = memory_read_byte(cpustate->program, 0x1e);
				cpustate->PS1 = memory_read_byte(cpustate->program, 0x1f);
				switch( irqline ) {
				case WDT_INT:
				...
				memory_write_byte(cpustate->program, 0x12, cpustate->IR0);
				memory_write_byte(cpustate->program, 0x13, cpustate->IR1);
			}
			irqline++;
		}
	}
...
static void sm8500_set_reg( sm8500_state *cpustate, int regnum, unsigned val )
{
	...
	case STATE_GENSP:
	case SM8500_SP:		cpustate->SP = val; memory_write_byte( cpustate->program, 0x1d, val&0xff); if (cpustate->SYS&0x40) memory_write_byte( cpustate->program, 0x1c, val>>8); break;
