...
#define COMMANDS_PER_BUCKET				16
#define CACHE_LINE_SIZE					64			/* this is a general guess */
#define TOTAL_BUCKETS					(512 / COMMANDS_PER_BUCKET)
#define UNITS_PER_COMMAND				(128 / COMMANDS_PER_BUCKET)
...
/* queue management */
static osd_work_queue *	queue;			/* work queue */

typedef struct _command_work_unit command_work_unit;
struct _command_work_unit
{
	...
	INT16				dummy0;					/* pad to 16 bytes */
#ifndef PTR64
	UINT32				dummy1;					/* pad to 16 bytes */
#endif
	UINT32	command[44];	// Command data
};

/* command work units */
static command_work_unit **	units;			/* array of work unit pointers */
static UINT32				unit_next;		/* index of next unit to allocate */
static UINT32				unit_count;		/* number of work units available */
static size_t				unit_size;		/* size of each work unit, in bytes */

/* buckets */
static UINT16				unit_bucket[TOTAL_BUCKETS]; /* buckets for tracking unit usage */
...
static void rdp_work_alloc(running_machine *machine, int max_commands)
{
	/* allocate command work units */
	int command_count = MAX(max_commands, 1);
	unit_size = sizeof(command_work_unit);
	unit_count = MIN(command_count * UNITS_PER_COMMAND, 65535);
	unit_next = 0;
	units = (command_work_unit **)allocate_array(&unit_size, unit_count);
	queue = osd_work_queue_alloc(WORK_QUEUE_FLAG_MULTI | WORK_QUEUE_FLAG_HIGH_FREQ);
}

//static void rdp_poly_wait(running_machine *machine)
//{
	/* wait for all pending work items to complete */
//	if (queue)
//	{
//		osd_work_queue_wait(queue, osd_ticks_per_second() * 100);
//	}

//	memset(unit_bucket, 0xff, sizeof(unit_bucket));
//}
...
static void *rdp_command_item_callback(void *param, int threadid)
{
	command_work_unit *unit = (command_work_unit *)param;
	UINT8 cmd = (unit->command[0] >> 24) & 0x3f;
	...
	while(1)
	{
		...
		{
			command_work_unit *prevunit = units[unit->previtem];
			if (prevunit->count_next != 0)
			{
				UINT32 unitnum = ((UINT8 *)unit - (UINT8 *)units[0]) / unit_size;
		...
		// execute the command
		rdp_command_table[cmd](unit->machine, unit->command[0], unit->command[1]);
...
{
	int i;
	UINT32 cmd, length, cmd_length, cur_cmd = 0;
	int startunit = unit_next;
	...
	{
		UINT32 bucketnum = ((UINT32)cur_cmd / COMMANDS_PER_BUCKET) % TOTAL_BUCKETS;
		UINT32 unit_index = unit_next++;
		UINT32 *data = units[unit_index]->command;
		int data_index = 0;
		units[unit_index]->machine = machine;
	...
	//  }
		...
		// spawn the work unit
		for(data_index = 0; data_index < rdp_command_length[cmd] / 4; data_index++)
		{
			data[data_index] = rdp_cmd_data[rdp_cmd_cur+data_index];
		}
		units[unit_index]->count_next = 0;
		units[unit_index]->previtem = unit_bucket[bucketnum];
		unit_bucket[bucketnum] = unit_index;

		rdp_cmd_cur += rdp_command_length[cmd] / 4;
	};

	osd_work_item_queue_multiple(queue, rdp_command_item_callback, unit_next - startunit, units[startunit], unit_size, WORK_ITEM_FLAG_AUTO_RELEASE);
	rdp_cmd_ptr = 0;
	rdp_cmd_cur = 0;
