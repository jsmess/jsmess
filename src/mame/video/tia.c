...
static WRITE8_HANDLER( RESM1_w )
{
	...
	if ( HMOVE_started != HMOVE_INACTIVE ) {
		...
		skipM1delay = ( curr_x < -1 && horzM1 % 160 >= 156 && horzM1 % 160 < 1 ) ? 4 : 0;
...
static WRITE8_HANDLER( RESMP0_w )
{
	if (RESMP0 & 2)
	{
		horzM0 = horzP0 + 3 * nusiz[NUSIZ0 & 7][1] - ( nusiz[NUSIZ0 & 7][1] > 1 ? 1 : 0 );
		if ( HMOVE_started != HMOVE_INACTIVE ) {
			horzM0 -= ( 8 - motclkP0 );
			horzM0 += 8 - motclkM0;
			if ( horzM0 < 0 )
...
static WRITE8_HANDLER( RESMP1_w )
{
	if (RESMP1 & 2)
	{
		horzM1 = horzP1 + 3 * nusiz[NUSIZ1 & 7][1] - ( nusiz[NUSIZ1 & 7][1] > 1 ? 1 : 0 );
		if ( HMOVE_started != HMOVE_INACTIVE ) {
			horzM1 -= ( 8 - motclkP1 );
			horzM1 += 8 - motclkM1;
			if ( horzM1 < 0 )
