...
static DEVICE_START(coco_cartridge)
{
	...
	const cococart_config *config = (const cococart_config *) downcast<const legacy_device_base *>(device)->inline_config();
	int i;
	...
	if (cartslot != NULL)
	{
		...
		cococart->pcb_r = (read8_device_func)downcast<const legacy_cart_slot_device_base *>(cococart->pcb)->get_legacy_fct(COCOCARTINFO_FCT_FF40_R);
		cococart->pcb_w = (write8_device_func)downcast<const legacy_cart_slot_device_base *>(cococart->pcb)->get_legacy_fct(COCOCARTINFO_FCT_FF40_W);
	}
...
static MACHINE_CONFIG_FRAGMENT( coco_cartridge )
	...
	MCFG_CARTSLOT_PCBTYPE(0, "",			COCO_CARTRIDGE_PCB_PAK)
	MCFG_CARTSLOT_PCBTYPE(1, "coco_fdc",	COCO_CARTRIDGE_PCB_FDC_COCO)
	MCFG_CARTSLOT_PCBTYPE(2, "banked_16k",	COCO_CARTRIDGE_PCB_PAK_BANKED16K)
	MCFG_CARTSLOT_PCBTYPE(3, "orch90",		COCO_CARTRIDGE_PCB_ORCH90)
	MCFG_CARTSLOT_PCBTYPE(4, "rs232",		COCO_CARTRIDGE_PCB_RS232)
