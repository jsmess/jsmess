	...
	switch(adb_state)
	{
		case ADBSTATE_IDLE:
			...
			switch(data)
			{
				...
				case 0x07:	/* synchronize */
					if (memory_region_length(machine, "|") == 0x40000)	/* HACK */
...
static READ8_HANDLER( apple2gs_c0xx_r )
{
	...
	switch(offset)
	{
		...
		case 0x7c: case 0x7d: case 0x7e: case 0x7f:
			offset |= (memory_region_length(machine, "|") - 1) & ~0x3FFF;
			result = memory_region(machine, "|")[offset];
			break;
...
static UINT8 *apple2gs_getslotmem(running_machine *machine, offs_t address)
{
	...
	rom += 0x030000 % memory_region_length(machine, "|");
	return &rom[address];
}
...
static void apple2gs_setup_memory(running_machine *machine)
{
	...
	begin = 0x1000000 - memory_region_length(machine, "|");
	end = 0xffffff;
	memory_install_read8_handler(machine, 0, ADDRESS_SPACE_PROGRAM, begin, end, 0, 0, SMH_BANK3);
	memory_set_bankptr(3, memory_region(machine, "|"));
