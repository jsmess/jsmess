...
{
	if (offset==0x00)	{	// 0x800 et 0x000=> vid??o page, HR
	...
	if (offset==0x04)	{	// 0x804 => vid??o page, BR
							hector_flag_hr=0;
                        	memory_set_bank(space->machine, "bank1", HECTOR_BANK_VIDEO);
							if (flag_clk ==0)
							{
...
else
{
    ...
    cassette_bit_mem = cassette_bit;  // M??morisation ??tat bit cassette
}
return value;
}
...
static void Init_Value_SN76477_Hector(void)
{
     ...
     // Version 3 : Ajust?? pour les frequences mesur??es :
                // 4  0 SOUND 255 Hz => ajust?? ?? l'oreille
                // 4  4 SOUND  65 Hz => ajust?? ?? l'oreille
                // 4  8 SOUND  17 Hz =>  ajust?? ?? l'oreille
                // 4 12 SOUND 4,3 Hz =>  ajust?? ?? l'oreille
     // SLF C       Version 3
     ...
     Pin_Value[24][0] = RES_K(1000);  //RES_M(1) infini sur Hector car non connect??e
     ...
     // Valeur corrig?? par rapport au sch??ma :
     Pin_Value[5][1] = RES_K(3.30 ) ;     // 330Kohm
     Pin_Value[5][0] = RES_K(1.76 ) ;     // 76 Kohm

     // Noise pas command?? par le bus audio !
	 // Seule la valeur [0] est document??e !
     ...
     // Initialisation ?? 0 des pin du SN
     AU[0]=0;
     AU[1]=0;
     AU[2]=0;
     AU[3]=0;
...
void Update_Sound(const address_space *space, UINT8 data)
{
	...
	sn76477_mixer_c_w(sn76477, ((ValMixer & 0x02)==2) ? 1 : 0);// Revu selon mesure ??lectronique
