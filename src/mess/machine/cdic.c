/******************************************************************************
    ...
    Philips CD-I Mono-I CD Controller Implementation (CDIC)
    -------------------
...
- Chip is, per system block diagrams, a 68HC05 which will need to be decapped
  for proper emulation.  Current emulation suffers from many bugs.
...
void register_globals(running_machine *machine)
{
    state_save_register_global(machine, m_command);
    state_save_register_global(machine, m_time);
    state_save_register_global(machine, m_file);
    state_save_register_global(machine, m_channel);
    state_save_register_global(machine, m_audio_channel);
    state_save_register_global(machine, m_audio_buffer);
    state_save_register_global(machine, m_x_buffer);
    state_save_register_global(machine, m_dma_control);
    state_save_register_global(machine, m_z_buffer);
    state_save_register_global(machine, m_interrupt_vector);
    state_save_register_global(machine, m_data_buffer);

    state_save_register_global(machine, m_audio_sample_freq);
    state_save_register_global(machine, m_audio_sample_size);
}
...
/*
** Decode a 16-bit mono XA stream
*/
...
/*
** Decode an 8-bit mono XA stream
*/
...
/*
** Decode a 16-bit stereo XA stream
*/
...
/*
** Decode an 8-bit stereo XA stream
*/
...
/*
** Decode a single buffered audio sector
*/
...
static void cdic_decode_audio_sector(running_machine *machine, const unsigned char *xa, int triggered)
{
	...
	switch(channels)
	{
		case 1:
			...
			{
				case 4:
                    cdic_decode_xa_mono(m_xa_last, hdr + 4, samples);
					...
					break;
				case 8:
                    cdic_decode_xa_mono8(m_xa_last, hdr + 4, samples);
					for(index = 18*28*8 - 1; index >= 0; index--)
					{
						samples[index*2 + 1] = samples[index];
						samples[index*2 + 0] = samples[index];
		...
		case 2:
			...
			{
				case 4:
                    cdic_decode_xa_stereo(m_xa_last, hdr + 4, samples);
					samples[18*28*8 + 0] = samples[18*28*8 + 2] = samples[18*28*8 + 4] = samples[18*28*8 + 6] = samples[18*28*8 + 8] = samples[18*28*8 + 10] = samples[18*28*8 + 12] = samples[18*28*8 + 14] = samples[18*28*8 - 2];
					samples[18*28*8 + 1] = samples[18*28*8 + 3] = samples[18*28*8 + 5] = samples[18*28*8 + 7] = samples[18*28*8 + 9] = samples[18*28*8 + 11] = samples[18*28*8 + 13] = samples[18*28*8 + 15] = samples[18*28*8 - 1];
                    //fwrite(samples, 1, 18*28*4*m_audio_sample_size, temp_adpcm);
					break;
				case 8:
                    cdic_decode_xa_stereo8(m_xa_last, hdr + 4, samples);
...
/*
** After an appropriate delay for decoding to take place, trigger playback
*/
...
TIMER_CALLBACK( audio_sample_trigger )
{
	...
	{
        verboselog(machine, 0, "Hit audio_sample_trigger, with m_decode_addr == %04x, calling cdic_decode_audio_sector\n", m_decode_addr );

		// Decode the data at Z+4, the same offset as a normal CD sector.
        cdic_decode_audio_sector(machine, ((UINT8*)m_ram) + (m_decode_addr & 0x3ffe) + 4, 1);
...
/*
** After an appropriate delay for processing to take place, trigger a data return.
*/
					...
					{
						{
                            ...
                            cdic_decode_audio_sector(machine, ((UINT8*)m_ram) + ((m_data_buffer & 5) * 0xa00 + 4), 0);
...
READ16_HANDLER( cdimono1_cdic_r )
{
...
WRITE16_HANDLER( cdimono1_cdic_w )
{
	cdi_state *state = space->machine->driver_data<cdi_state>();
	cdic_regs_t *cdic = &state->cdic_regs;
...
READ16_HANDLER( cdimono1_cdic_ram_r )
{
    cdi_state *state = space->machine->driver_data<cdi_state>();
    cdic_regs_t *cdic = &state->cdic_regs;
...
WRITE16_HANDLER( cdimono1_cdic_ram_w )
{
    cdi_state *state = space->machine->driver_data<cdi_state>();
    cdic_regs_t *cdic = &state->cdic_regs;
