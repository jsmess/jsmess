...
/*
static void dmux_unmount_device(running_device *dmuxdev, running_device *busdevice)
{
	datamux_state *dmux = get_safe_token(dmuxdev);
	for (int index = 0; index < MAXDEV; index++)
	{
		if (dmux->component[index].device == busdevice)
		{
			dmux->component[index].device = NULL;
			return;
		}		
	}
	printf("Failed to uninstall device from dmux\n");
}
*/
/***************************************************************************
