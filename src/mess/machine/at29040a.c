...
static TIMER_CALLBACK(at29c040a_programming_timer_callback)
{
	...
	case s_pgm_2:
		...
		timer_adjust_oneshot(feeprom->programming_timer, ATTOTIME_IN_MSEC(5), 0);
		break;
...
READ8_DEVICE_HANDLER( at29c040a_r )
{
	...
	if (feeprom->s_pgm == s_pgm_1)
	{
		...
		feeprom->s_enabling_sdb = FALSE;
		feeprom->s_disabling_sdb = FALSE;
		timer_adjust_oneshot(feeprom->programming_timer, attotime_never, 0);
	}
...
WRITE8_DEVICE_HANDLER( at29c040a_w )
{
		...
		if (cfi_offset == 0x5555)
		{
			...
			case 0x20:
				...
				if (feeprom->s_cmd_0x80_flag)
				{
					...
					//timer_adjust_periodic(feeprom->programming_timer, ATTOTIME_IN_USEC(150), id, 0.);
				}
				break;
	...
	if (feeprom->s_pgm == s_pgm_2)
	{
		...
		timer_adjust_oneshot(feeprom->programming_timer, ATTOTIME_IN_USEC(150), 0);
	}
}
...
{
	at29c040a_state *feeprom = get_safe_token(device); 
	feeprom->programming_timer = timer_alloc(device->machine, at29c040a_programming_timer_callback, (void *)device);
}
