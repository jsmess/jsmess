			...
			if ( state->m_prc.copy_enabled )
			{
				...
				for( y = 0; y < 64; y += 8 ) {
					for( x = 0; x < 96; x++ ) {
						...
						machine.generic.tmpbitmap->pix16(y + 0, x) = ( data & 0x01 ) ? 3 : 0;
						machine.generic.tmpbitmap->pix16(y + 1, x) = ( data & 0x02 ) ? 3 : 0;
						machine.generic.tmpbitmap->pix16(y + 2, x) = ( data & 0x04 ) ? 3 : 0;
						machine.generic.tmpbitmap->pix16(y + 3, x) = ( data & 0x08 ) ? 3 : 0;
						machine.generic.tmpbitmap->pix16(y + 4, x) = ( data & 0x10 ) ? 3 : 0;
						machine.generic.tmpbitmap->pix16(y + 5, x) = ( data & 0x20 ) ? 3 : 0;
						machine.generic.tmpbitmap->pix16(y + 6, x) = ( data & 0x40 ) ? 3 : 0;
						machine.generic.tmpbitmap->pix16(y + 7, x) = ( data & 0x80 ) ? 3 : 0;
					}
				}
