...
READ8_HANDLER( dai_io_discrete_devices_r )
{
	...
	case 0x00:
		...
		if (cassette_input(space->machine().device(CASSETTE_TAG)) > 0.01)
...
WRITE8_HANDLER( dai_io_discrete_devices_w )
{
	...
	case 0x06:
		...
		cassette_change_state(space->machine().device(CASSETTE_TAG), state->m_cassette_motor[0]?CASSETTE_MOTOR_DISABLED:CASSETTE_MOTOR_ENABLED, CASSETTE_MASK_MOTOR);
