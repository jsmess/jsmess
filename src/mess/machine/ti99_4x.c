...
DRIVER_INIT( ti99_4 )
{
	...
	xRAM_ptr = (UINT16 *) (memory_region(machine, "|") + offset_xram);
...
DRIVER_INIT( ti99_4p )
{
	...
	/* set up memory pointers */
	xRAM_ptr = (UINT16 *) (memory_region(machine, "|") + offset_xram_4p);
	/*console_GROMs.data_ptr = memory_region(machine, region_grom);*/
}
...
DEVICE_START( ti99_cart )
{
	...
	cartridge_pages[id] = (UINT16 *) (memory_region(device->machine, "|") + offset_cart + (id * 0x2000));
}
...
MACHINE_RESET( ti99 )
{
	...
	else
	{
		...
		memory_set_bankptr(1, memory_region(machine, "|") + offset_sram);
	}
...
WRITE16_HANDLER ( ti99_4p_cart_w )
{
	if (ti99_4p_internal_rom6_enable)
	{
		ti99_4p_internal_ROM6 = (UINT16 *) (memory_region(machine, "|") + (FPTR)((offset & 1) ? offset_rom6b_4p : offset_rom6_4p));
		return;
	}
		...
		else if ((offset >= 0x8000) && (offset < 0xa000))
		{
			...
			case 5:
				...
				if (! (offset & 1))
				{
					...
					{
						attotime time_to_ready = double_to_attotime(tms5220_time_to_ready());
						double d = ceil(ATTOTIME_TO_CYCLES(0, time_to_ready));
...
static void ti99_8_internal_dsr_reset(running_machine *machine)
{
	ti99_8_internal_DSR = memory_region(machine, "|") + offset_rom0_8 + 0x4000;
