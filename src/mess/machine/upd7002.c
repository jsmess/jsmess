	...
	switch(offset&0x03)
	{
		...
		*/
		...
		if (uPD7002_status&0x08)
		{
			...
			timer_set(space->machine, ATTOTIME_IN_MSEC(10), NULL, conversion_counter, uPD7002_conversioncomplete);
		} else {
			// 8 bit conversion takes 4ms
			timer_set(space->machine, ATTOTIME_IN_MSEC(4),  NULL, conversion_counter, uPD7002_conversioncomplete);
		}
		break;
