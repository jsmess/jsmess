	...
	switch (offset)
	{
		case ACIA_6850_CTRL:
			if ((*(*currptr).intf).in_status_func)
							val = (*(*currptr).intf).in_status_func(space, 0);
			break;
		case ACIA_6850_DATA:
			if ((*(*currptr).intf).in_recv_func)
							val = (*(*currptr).intf).in_recv_func(space, 0);
			break;
	}
	return (val);
}
	...
	switch (offset)
	{
		case ACIA_6850_CTRL:
			if ((*(*currptr).intf).out_status_func)
							(*(*currptr).intf).out_status_func(space, 0, data);
			break;
		case ACIA_6850_DATA:
			if ((*(*currptr).intf).out_tran_func)
							(*(*currptr).intf).out_tran_func(space, 0, data);
			break;
	}
}

 READ8_HANDLER( acia6850_0_r ) { return acia6850_read (space, 0, offset); }
 READ8_HANDLER( acia6850_1_r ) { return acia6850_read (space, 1, offset); }
 READ8_HANDLER( acia6850_2_r ) { return acia6850_read (space, 2, offset); }
 READ8_HANDLER( acia6850_3_r ) { return acia6850_read (space, 3, offset); }

WRITE8_HANDLER( acia6850_0_w ) { acia6850_write (space, 0, offset, data); }
WRITE8_HANDLER( acia6850_1_w ) { acia6850_write (space, 1, offset, data); }
WRITE8_HANDLER( acia6850_2_w ) { acia6850_write (space, 2, offset, data); }
WRITE8_HANDLER( acia6850_3_w ) { acia6850_write (space, 3, offset, data); }
