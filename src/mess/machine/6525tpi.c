...
static DEVICE_SET_INFO( tpi6525 )
{
	switch (state)
	{
...
DEVICE_GET_INFO( tpi6525 )
{
	switch (state)
	{
		...
		case DEVINFO_FCT_SET_INFO:				info->set_info = DEVICE_SET_INFO_NAME( tpi6525 );	break;
...
static void tpi6525_set_interrupt(const device_config *device)
{
	...
	if (!tpi6525->interrupt_level && (tpi6525->air != 0))
	{
		...
		DBG_LOG(3, "tpi6525", ("%s set interrupt\n", device->tag));
...
static void tpi6525_clear_interrupt(const device_config *device)
{
	...
	if (tpi6525->interrupt_level && (tpi6525->air == 0))
	{
		...
		DBG_LOG (3, "tpi6525", ("%s clear interrupt\n", device->tag));
...
READ8_DEVICE_HANDLER( tpi6525_r )
{
	...
	case 2:
		...
		DBG_LOG(2, "tpi6525", ("%s read %.2x %.2x\n", device->tag, offset, data));
		break;
	...
	DBG_LOG(3, "tpi6525", ("%s read %.2x %.2x\n", device->tag, offset, data));
...
WRITE8_DEVICE_HANDLER( tpi6525_w )
{
	...
	DBG_LOG(2, "tpi6525", ("%s write %.2x %.2x\n", device->tag, offset, data));
