/**********************************************************************
    ...
    8279 DMA interface and emulation
...
#define DMA8279_VERIFY_TRANSFER		0x00
#define DMA8279_WRITE_TRANSFER		0x04
#define DMA8279_READ_TRANSFER		0x08
#define DMA8279_ILLEGAL_TRANSFER	0x0c

#define DMA8279_DEMAND_MODE		0x00
#define DMA8279_SINGLE_MODE		0x40
#define DMA8279_BLOCK_MODE		0x80
#define DMA8279_CASCADE_MODE	0xc0
...
void i8279_device::device_start()
{
	...
	m_timer = machine().scheduler().timer_alloc(FUNC(i8279_timerproc_callback), (void *)this);
}
...
void i8279_device::i8279_advance()
{
	int channel = m_service_channel;
...
void i8279_device::i8327_set_dack(int channel)
{
	for (int i = 0; i < 4; i++)
	{
...
TIMER_CALLBACK( i8279_device::i8279_timerproc_callback )
{
	reinterpret_cast<i8279_device*>(ptr)->i8279_timerproc();
}


void i8279_device::i8279_timerproc()
{
	...
	case DMA8279_S0:
		...
		if ( m_hlda )
		{
			if ( DMA_MODE_TRANSFERMODE( m_chan[m_service_channel].m_mode ) == DMA8279_CASCADE_MODE )
			{
	...
	case DMA8279_SC:	/* Cascade mode, waiting until peripheral is done */
	...
	case DMA8279_S1:	/* Output A8-A15 */
		m_state = DMA8279_S2;
		break;

	case DMA8279_S2:	/* Output A7-A0 */
		/* set DACK */
		i8327_set_dack(m_service_channel);
	...
	case DMA8279_S3:	/* Initiate read */
		i8279_do_read();
		m_state = DMA8279_S4;
		break;

	case DMA8279_S4:	/* Perform read/write */
...

		{
			...
			case DMA8279_DEMAND_MODE:
				...
				{
					m_state = m_chan[channel].m_high_address_changed ? DMA8279_S1 : DMA8279_S2;
				}
				break;

			case DMA8279_SINGLE_MODE:
				m_hrq = 0;
				m_hlda = 0;
				m_out_hrq_func(m_hrq);
				m_state = DMA8279_SI;
		...
		/* clear DACK */
		if ( m_state == DMA8279_SI )
		{
			i8327_set_dack(-1);
		}
		break;

	case DMA8279_S11: /* Output A8-A15 */

//      logerror("###### dma8279_timerproc %s: from %04x count=%x to %04x count=%x\n", tag(),
...
//              m_chan[1].m_address, m_chan[1].m_count);
		...
		//  logerror("!!! dma8279_timerproc DMA8279_S11 %s: m_drq=%x m_command=%x\n", tag(), m_drq, m_command);
		}
		break;
	}
}
...
READ8_DEVICE_HANDLER_TRAMPOLINE(i8279, i8279_r)
{
	UINT8 data = 0xFF;
...
WRITE8_DEVICE_HANDLER_TRAMPOLINE(i8279, i8279_w)
{
	offset &= 0x0F;

//  logerror("i8279_w: offset = %02x, data = %02x\n", offset, data );
...
void i8279_device::i8279_drq_write(int channel, int state)
{
	if (state)
	{
		m_drq |= ( 0x01 << channel );
...
WRITE_LINE_DEVICE_HANDLER( i8279_hlda_w ) { downcast<i8279_device*>(device)->i8279_hlda_w(state); }
WRITE_LINE_DEVICE_HANDLER( i8279_ready_w ) { }
WRITE_LINE_DEVICE_HANDLER( i8279_dreq0_w ) { downcast<i8279_device*>(device)->i8279_drq_write(0, state); }
WRITE_LINE_DEVICE_HANDLER( i8279_dreq1_w ) { downcast<i8279_device*>(device)->i8279_drq_write(1, state); }
WRITE_LINE_DEVICE_HANDLER( i8279_dreq2_w ) { downcast<i8279_device*>(device)->i8279_drq_write(2, state); }
WRITE_LINE_DEVICE_HANDLER( i8279_dreq3_w ) { downcast<i8279_device*>(device)->i8279_drq_write(3, state); }
