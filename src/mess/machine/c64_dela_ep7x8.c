...
ROM_START( c64_dela_ep7x8 )
	ROM_REGION( 0x10000, "roml", 0 )
	ROM_LOAD( "dela_7x8k-eprom-bank.bin", 0x0000, 0x2000, CRC(546d7ea8) SHA1(6ab7a279144bcaaa20e7b2d12d67d3ed5cc3f6b6) )
...
UINT8 c64_dela_ep7x8_cartridge_device::c64_cd_r(address_space &space, offs_t offset, int roml, int romh, int io1, int io2)
{
	...
	if (!roml)
	{
		...
		if (!BIT(m_bank, 0)) data |= m_roml[0x0000 + addr];
		if (!BIT(m_bank, 1)) data |= m_roml[0x2000 + addr];
		if (!BIT(m_bank, 2)) data |= m_roml[0x4000 + addr];
		if (!BIT(m_bank, 3)) data |= m_roml[0x6000 + addr];
		if (!BIT(m_bank, 4)) data |= m_roml[0x8000 + addr];
		if (!BIT(m_bank, 5)) data |= m_roml[0xa000 + addr];
		if (!BIT(m_bank, 6)) data |= m_roml[0xc000 + addr];
		if (!BIT(m_bank, 7)) data |= m_roml[0xe000 + addr];
	}
...
void c64_dela_ep7x8_cartridge_device::c64_cd_w(address_space &space, offs_t offset, UINT8 data, int roml, int romh, int io1, int io2)
{
	if (!io1)
	{
		m_bank = data;
