...
static void gb_init_regs(running_machine *machine) {
	...
	gb_io_w( cpu_get_address_space(machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x05, 0x00 );		/* TIMECNT */
	gb_io_w( cpu_get_address_space(machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x06, 0x00 );		/* TIMEMOD */
}
...
static void gb_init(running_machine *machine) {
	...
	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x16, 0x00 );       /* Initialize sound hardware */
...
MACHINE_RESET( sgb )
{
	...
	gb_sound_w(devtag_get_device(machine, SOUND_GAMEBOY, "custom"), 0x16,0xF0);	/* F0 for SGB */
	gb_sound_w(devtag_get_device(machine, SOUND_GAMEBOY, "custom"), 0x15,0xF3);
	gb_sound_w(devtag_get_device(machine, SOUND_GAMEBOY, "custom"), 0x14,0x77);
...
MACHINE_RESET( gbpocket )
{
	...
	/* Initialize the Sound registers */
	gb_sound_w(devtag_get_device(machine, SOUND_GAMEBOY, "custom"), 0x16,0x80);
	gb_sound_w(devtag_get_device(machine, SOUND_GAMEBOY, "custom"), 0x15,0xF3);
	gb_sound_w(devtag_get_device(machine, SOUND_GAMEBOY, "custom"), 0x14,0x77);
...
MACHINE_RESET( gbc )
{
	...
	gbc_io2_w( cpu_get_address_space( machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x30, 0x00 );

	/* Initialise registers */
	gb_io_w( cpu_get_address_space( machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x6C, 0xFE );
	gb_io_w( cpu_get_address_space( machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x72, 0x00 );
	gb_io_w( cpu_get_address_space( machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x73, 0x00 );
	gb_io_w( cpu_get_address_space( machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x74, 0x8F );
	gb_io_w( cpu_get_address_space( machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x75, 0x00 );
	gb_io_w( cpu_get_address_space( machine->cpu[0], ADDRESS_SPACE_PROGRAM ), 0x76, 0x00 );
...
static void gb_machine_stop(running_machine *machine)
{
	...
	image_battery_save(devtag_get_device(machine, CARTSLOT, "cart"), gb_cart_ram, RAMBanks * 0x2000 );
}
...
WRITE8_HANDLER ( gb_io_w )
{
	...
	case 0x0F:						/* IF - Interrupt flag */
		data &= 0x1F;
		cpu_set_reg( space->machine->cpu[0], LR35902_IF, data );
		break;
	}
...
{
	return cpu_get_reg( space->machine->cpu[0], LR35902_IE );
}

WRITE8_HANDLER ( gb_ie_w )
{
	cpu_set_reg( space->machine->cpu[0], LR35902_IE, data & 0x1F );
}
...
READ8_HANDLER ( gb_io_r )
{
	switch(offset)
	{
		...
		case 0x0F:
			...
			return 0xE0 | cpu_get_reg( space->machine->cpu[0], LR35902_IF );
		default:
...
static TIMER_CALLBACK(gb_serial_timer_proc)
{
	...
	if ( ! SIOCount ) {
		SIOCONT &= 0x7F;
		timer_enable( gb_serial_timer, 0 );
		cpu_set_input_line(machine->cpu[0], SIO_INT, HOLD_LINE);
	}
}
...
INLINE void gb_timer_check_irq( running_machine *machine ) {
	...
	if ( gb_timer.triggering_irq ) {
		...
		if ( TIMECNT == 0 ) {
			TIMECNT = TIMEMOD;
			cpu_set_input_line(machine->cpu[0], TIM_INT, HOLD_LINE );
...
WRITE8_HANDLER ( gbc_io2_w )
{
	switch( offset ) {
		case 0x0D:	/* KEY1 - Prepare speed switch */
			cpu_set_reg( space->machine->cpu[0], LR35902_SPEED, data );
			return;
...
READ8_HANDLER( gbc_io2_r ) {
	switch( offset ) {
	case 0x0D:	/* KEY1 */
		return cpu_get_reg( space->machine->cpu[0], LR35902_SPEED );
	case 0x16:	/* RP - Infrared port */
		break;
	default:
		break;
...
{
	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), megaduck_sound_offsets[offset], data );
}

READ8_HANDLER( megaduck_sound_r1 )
{
	return gb_sound_r( devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), megaduck_sound_offsets[offset] );
}

WRITE8_HANDLER( megaduck_sound_w2 )
{
	switch(offset) {
		case 0x00:	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x10, data );	break;
		case 0x01:	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x12, data );	break;
		case 0x02:	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x11, data );	break;
		case 0x03:	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x13, data );	break;
		case 0x04:	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x14, data );	break;
		case 0x05:	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x16, data );	break;
		case 0x06:	gb_sound_w(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x15, data );	break;
		case 0x07:
		case 0x08:
		case 0x09:
		case 0x0A:
...
{
	return gb_sound_r(devtag_get_device(space->machine, SOUND_GAMEBOY, "custom"), 0x10 + megaduck_sound_offsets[offset]);
}
