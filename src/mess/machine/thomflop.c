...
static READ8_HANDLER ( to7_5p14_r )
{
	...
	if ( offset < 4 )
		...
		logerror ( "%f $%04x to7_5p14_r: invalid read offset %i\n", timer_get_time(space->machine).as_double(), cpu_get_previouspc(space->machine->device("maincpu")), offset );
	return 0;
}
...
static WRITE8_HANDLER( to7_5p14_w )
{
	...
	else if ( offset == 8 )
	{
		...
		default:
			logerror( "%f $%04x to7_5p14_w: invalid drive select pattern $%02X\n", timer_get_time(space->machine).as_double(), cpu_get_previouspc(space->machine->device("maincpu")), data );
		}
		...
		if ( drive != -1 )
		{
			...
			LOG(( "%f $%04x to7_5p14_w: $%02X set drive=%i side=%i density=%s\n",
			      timer_get_time(space->machine).as_double(), cpu_get_previouspc(space->machine->device("maincpu")),
			      data, drive, side, (BIT(data, 7) ? "FM" : "MFM")));
		}
	}
	else
		logerror ( "%f $%04x to7_5p14_w: invalid write offset %i (data=$%02X)\n",
			   timer_get_time(space->machine).as_double(), cpu_get_previouspc(space->machine->device("maincpu")), offset, data );
}
	...
	else if ( offset >= 8 && offset <= 9 )
	{
		...
		if ( drive != -1 )
		{
			...
			LOG(( "%f $%04x to7_5p14sd_w: $%02X set drive=%i side=%i\n",
			      timer_get_time(space->machine).as_double(), cpu_get_previouspc(space->machine->device("maincpu")), data, drive, side ));
		}
	}
	else
...
static void to7_qdd_index_pulse_cb ( device_t *controller,device_t *image, int state )
{
	...
	VLOG(( "%f to7_qdd_pulse_cb: state=%i\n", timer_get_time(image->machine).as_double(), state ));
}
...
static UINT8 to7_qdd_read_byte( running_machine *machine )
{
	...
	VLOG(( "%f $%04x to7_qdd_read_byte: RDATA off=%i/%i data=$%02X\n",
	       timer_get_time(machine).as_double(), cpu_get_previouspc(machine->device("maincpu")),
...
static void to7_qdd_write_byte( running_machine *machine, UINT8 data )
{
	     ...
	     to7qdd->data_idx <to7qdd->data_size )
	{
		...
		VLOG (( "%f $%04x to7_qdd_write_byte: got $%02X offs=%i-%i\n",
			timer_get_time(machine).as_double(), cpu_get_previouspc(machine->device("maincpu")), data,
			to7qdd->start_idx, to7qdd->data_idx ));
		     ...
		     to7qdd->data[ to7qdd->start_idx + 3 ] == to7qdd->data_crc )
		{
			...
			LOG(( "%f $%04x to7_qdd_write_byte: got id field for sector=%i\n",
			      timer_get_time(machine).as_double(), cpu_get_previouspc(machine->device("maincpu")), sector ));
...
static READ8_HANDLER ( to7_qdd_r )
{
	...
	case 0: /* MC6852 status */
		...
		VLOG(( "%f $%04x to7_qdd_r: STAT=$%02X irq=%i pe=%i ovr=%i und=%i tr=%i rd=%i ncts=%i\n",
		       timer_get_time(space->machine).as_double(), cpu_get_previouspc(space->machine->device("maincpu")), to7qdd->status,
...
static void thmfc_floppy_cmd_complete( running_machine *machine )
{
	LOG (( "%f thmfc_floppy_cmd_complete_cb: cmd=%i off=%i/%i/%i\n",
	       timer_get_time(machine).as_double(), thmfc1->op, thmfc1->data_idx,
...
READ8_HANDLER ( thmfc_floppy_r )
{
	...
	case 0: /* STAT0 */
		...
		VLOG(( "%f $%04x thmfc_floppy_r: STAT0=$%02X\n", timer_get_time(space->machine).as_double(), cpu_get_previouspc(space->machine->device("maincpu")), thmfc1->stat0 ));
		return thmfc1->stat0;
...
static TIMER_CALLBACK( ans3 )
{
	...
	timer_set( machine, attotime::from_usec( 100 ), NULL, 0, ans4 );
}
...
static TIMER_CALLBACK( ans2 )
{
	LOG(( "%f ans2\n", timer_get_time(machine).as_double() ));
	mc6854_set_cts( machine->device("mc6854"), 0 );
	timer_set( machine, attotime::from_usec( 100 ), NULL, 0, ans3 );
}
...
static TIMER_CALLBACK( ans )
{
	LOG(( "%f ans\n", timer_get_time(machine).as_double() ));
	mc6854_set_cts( machine->device("mc6854"), 1 );
	timer_set( machine, attotime::from_usec( 100 ), NULL, 0, ans2 );
}
/* consigne DKBOOT
...
{
	int i;
	LOG(( "%f to7_network_got_frame:", timer_get_time(device->machine).as_double() ));
	for ( i = 0; i < length; i++ )
		LOG(( " $%02X", data[i] ));
	LOG(( "\n" ));
	...
	if ( data[1] == 0xff )
	{
		...
		timer_set( device->machine, attotime::from_usec( 100 ), NULL, 0, ans  );
...
static READ8_HANDLER ( to7_network_r )
{
	...
	if ( offset == 8 )
	{
		...
		VLOG(( "%f $%04x to7_network_r: read id $%02X\n", timer_get_time(space->machine).as_double(), cpu_get_previouspc(space->machine->device("maincpu")), id ));
		return id;
	}
