...
static void vii_blit_page(running_machine *machine, bitmap_t *bitmap, const rectangle *cliprect, int depth, UINT32 bitmap_addr, UINT16 *regs)
{
	...
	for(y0 = 0; y0 < 240; y0++)
	{
		for(x0 = 0; x0 < 320; x0++)
		{
			*BITMAP_ADDR32(bitmap, y0, x0) = (vii_screen_r[x0 + 320*y0] << 16) | (vii_screen_g[x0 + 320*y0] << 8) | vii_screen_b[x0 + 320*y0];
		}
	}
}
...
static READ16_HANDLER( vii_video_r )
{
	switch(offset)
	{
		...
		default:
			verboselog(space->machine, 0, "vii_video_r: Unknown register %04x\n", 0x2800 + offset);
			break;
	}
	return 0;
}
...
static READ16_HANDLER( vii_audio_r )
{
	...
	{
		default:
			verboselog(space->machine, 0, "vii_audio_r: Unknown register %04x\n", 0x3000 + offset);
			break;
	}
	return 0;
}
...
static WRITE16_HANDLER( vii_audio_w )
{
	...
	{
		default:
			verboselog(space->machine, 0, "vii_audio_w: Unknown register %04x = %04x (%04x)\n", 0x3000 + offset, data, mem_mask);
			break;
	}
}
...
static READ16_HANDLER( vii_io_r )
{
	...
	{
		default:
			verboselog(space->machine, 0, "vii_io_r: Unknown register %04x\n", 0x3800 + offset);
			break;
	}
	return 0;
}
...
static WRITE16_HANDLER( vii_io_w )
{
	...
	{
		default:
			verboselog(space->machine, 0, "vii_io_w: Unknown register %04x = %04x (%04x)\n", 0x3800 + offset, data, mem_mask);
			break;
	}
}
...
static ADDRESS_MAP_START( vii_mem, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE( 0x005200, 0x0055ff ) AM_RAM_WRITE(vii_rowscroll_w) AM_BASE(&vii_rowscroll)
	AM_RANGE( 0x005600, 0x0057ff ) AM_RAM AM_BASE(&vii_palette)
	AM_RANGE( 0x005800, 0x005fff ) AM_RAM_WRITE(vii_spriteram_w) AM_BASE(&vii_spriteram)
...
static INTERRUPT_GEN( vii_vblank )
{
	...
	which ^= 3;
	verboselog(device->machine, 0, "IRQ\n");
	if(VII_VIDEO_IRQ_STATUS)
	{
...
static MACHINE_DRIVER_START( vii )
	MDRV_CPU_ADD( "maincpu", UNSP, XTAL_27MHz / 2) // Arbitrary divisor
