...
INPUT_PORTS_START( b2m )
	...
	PORT_START /* line 7 */
		...
		PORT_BIT(0x04, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("LCtrl") PORT_CODE(KEYCODE_LCONTROL)
		PORT_BIT(0x08, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("RCtrl") PORT_CODE(KEYCODE_RCONTROL)
...
ROM_START( b2m )
    ...
    ROM_LOAD( "b2m.rom", 0x10000, 0x2000, CRC(3F3214D6) )
ROM_END

ROM_START( b2mrom )
    ROM_REGION( 0x22000, REGION_CPU1, ROMREGION_ERASEFF )
    ROM_LOAD( "bios2.rom", 0x10000, 0x2000, CRC(C22A98B7) )
    ROM_LOAD( "ramdos.sys", 0x12000, 0x60c0, CRC(91ED6DF0) )
ROM_END

SYSTEM_CONFIG_START(b2m)	
