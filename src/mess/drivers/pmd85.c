...
static ADDRESS_MAP_START( pmd85_mem , ADDRESS_SPACE_PROGRAM, 8)
	...
	AM_RANGE(0x4000, 0x7fff) AM_READWRITE(SMH_BANK5, SMH_BANK5)
	AM_RANGE(0x8000, 0x8fff) AM_READWRITE(SMH_BANK6, SMH_ROM)
	AM_RANGE(0x9000, 0x9fff) AM_READWRITE(SMH_NOP,   SMH_NOP)
	AM_RANGE(0xa000, 0xafff) AM_READWRITE(SMH_BANK7, SMH_ROM)
	AM_RANGE(0xb000, 0xbfff) AM_READWRITE(SMH_NOP,   SMH_NOP)
	AM_RANGE(0xc000, 0xffff) AM_READWRITE(SMH_BANK8, SMH_BANK8)
ADDRESS_MAP_END
...
static ADDRESS_MAP_START( pmd852a_mem , ADDRESS_SPACE_PROGRAM, 8)
	...
	AM_RANGE(0x4000, 0x7fff) AM_READWRITE(SMH_BANK5, SMH_BANK5)
	AM_RANGE(0x8000, 0x8fff) AM_READWRITE(SMH_BANK6, SMH_ROM)
	AM_RANGE(0x9000, 0x9fff) AM_READWRITE(SMH_BANK7, SMH_BANK7)
	AM_RANGE(0xa000, 0xafff) AM_READWRITE(SMH_BANK8, SMH_ROM)
	AM_RANGE(0xb000, 0xbfff) AM_READWRITE(SMH_BANK9, SMH_BANK9)
	AM_RANGE(0xc000, 0xffff) AM_READWRITE(SMH_BANK10, SMH_BANK10)
ADDRESS_MAP_END
...
static ADDRESS_MAP_START( alfa_mem , ADDRESS_SPACE_PROGRAM, 8)
	AM_RANGE(0x0000, 0x0fff) AM_READWRITE(SMH_BANK1, SMH_BANK1)
	AM_RANGE(0x1000, 0x33ff) AM_READWRITE(SMH_BANK2, SMH_BANK2)
	AM_RANGE(0x3400, 0x3fff) AM_READWRITE(SMH_BANK3, SMH_BANK3)
	AM_RANGE(0x4000, 0x7fff) AM_READWRITE(SMH_BANK4, SMH_BANK4)
	AM_RANGE(0x8000, 0x8fff) AM_READWRITE(SMH_BANK5, SMH_ROM)
	AM_RANGE(0x9000, 0xb3ff) AM_READWRITE(SMH_BANK6, SMH_ROM)
	AM_RANGE(0xb400, 0xbfff) AM_READWRITE(SMH_NOP,   SMH_NOP)
	AM_RANGE(0xc000, 0xffff) AM_READWRITE(SMH_BANK7, SMH_BANK7)
ADDRESS_MAP_END
...
static INPUT_PORTS_START( pmd85 )
	...
	PORT_START_TAG("KEY10") /* port 0x0a */	
	...
	PORT_START_TAG("KEY11") /* port 0x0b */	
...
static INPUT_PORTS_START (mato)
	...
	PORT_START_TAG("KEY8") /* port 0x08 */
		...
		PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_UNUSED )
	PORT_START_TAG("RESET") /* port 0x09 */
		PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("RST") PORT_CODE(KEYCODE_BACKSPACE) PORT_CHAR(UCHAR_MAMEKEY(BACKSPACE))
INPUT_PORTS_END
