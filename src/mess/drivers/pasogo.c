...
static WRITE8_HANDLER( vg230_io_w )
{
  int log=TRUE;
	...
	break;
      case 0x7a:
	if (data&2) { vg230.rtc.alarm_interrupt_request=FALSE; vg230.rtc.onehertz_interrupt_request=FALSE; /* update interrupt */ }
	break;
      }
      if (log)
...
static int device_load_pasogo_cart(mess_image *image)
{
...
static void pasogo_cartslot_getinfo(const mess_device_class *devclass, UINT32 state, union devinfo *info)
{
	...
	switch(state)
	{
		...
		case MESS_DEVINFO_PTR_LOAD:							info->load = device_load_pasogo_cart; break;
