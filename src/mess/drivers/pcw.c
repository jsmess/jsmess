...
static void pcw_update_mem(running_machine *machine, int block, int data)
{
	...
	if ((pcw_boot) && (block==0))
	{
		...
		FakeROM = &memory_region(Machine, REGION_CPU1)[0x010000];
...
static INPUT_PORTS_START(pcw)
	...
	PORT_START_TAG("LINE14")	/* 0x03ffe */
	PORT_BIT ( 0xff, 0x00,	 IPT_UNUSED )

	PORT_START_TAG("LINE15")	/* 0x03fff */
	PORT_BIT ( 0xff, 0x00,	 IPT_UNUSED )
