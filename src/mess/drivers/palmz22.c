...
static UINT32 s3c2410_gpio_port_r( device_t *device, int port, UINT32 mask)
{
	...
	switch (port)
	{
		...
		{
			data = (data & ~0xFF) | (input_port_read( device->machine(), "PORT-F") & 0xFF);
		}
		break;
...
static READ32_DEVICE_HANDLER( s3c2410_adc_data_r )
{
	...
	switch (offset)
	{
		...
		case 2 + 1 : data = 0x3FF - input_port_read( device->machine(), "PENY"); break;
	}
...
COMP(2005, palmz22, 0, 0, palmz22, palmz22, palmz22, "Palm", "Palm Z22", GAME_NO_SOUND)
