...
static TIMER_DEVICE_CALLBACK( keyboard_t1_tick )
{
	...
	state->kb_clk = !state->kb_clk;

	z80dart_rxtxcb_w(state->z80dart, state->kb_clk);
}
...
static READ8_HANDLER( keyboard_col_r )
{
	...
	if (state->kb_stb && state->kb_row < 12)
	{
		data = input_port_read(space->machine, ABC800_KEY_ROW[state->kb_row]);
	}
...
static WRITE8_HANDLER( keyboard_ctrl_w )
{
	...
	*/
	...
	// TxD
	state->kb_txd = !BIT(data, 4);

	// keydown
	z80dart_dcdb_w(state->z80dart, !BIT(data, 5));

	// strobe
	state->kb_stb = BIT(data, 6);
}

static READ8_HANDLER( keyboard_t1_r )
{
	abc800_state *state = space->machine->driver_data<abc800_state>();
...
static MACHINE_START( abc800 )
{
	...
	state_save_register_global(machine, state->kb_stb);
