...
ROM_START( sdk86 )
	...
	// Note that the roms at FE000-FEFFF and FF000-FFFFF are interchangable; the one at FF000-FFFFF is the one which starts on bootup, and the other one lives at FE000-FEFFF and can be switched in by the user. One is the Serial RS232 Monitor and one is the Keypad/front panel monitor. On the SDK-86 I (LN) dumped, the Keypad monitor was primary, but the other SDK-86 I know of has the roms in the opposite arrangement (Serial primary).
	// Serial Monitor Version ? (have not tried it)
