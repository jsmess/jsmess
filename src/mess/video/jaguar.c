...
static void jaguar_set_palette(UINT16 vmode)
{
	...
	switch (vmode & 0x106)
	{
		...
		/* YCC/RGB VARMOD */
		case 0x100:	// CRY16, varmod on
		case 0x106:	// RGB16, varmod on
			for (i = 0; i < 65536; i++)
			{
				UINT8 r = (red_lookup[i >> 8] * (i & 0xff)) >> 8;
				UINT8 g = (grn_lookup[i >> 8] * (i & 0xff)) >> 8;
...
#endif
				int hperiod = ((gpu_regs[HP] & 0x3ff) + 1)<<1;
				int hbend = MAX(gpu_regs[HDB1], gpu_regs[HBE]);
				int hbstart = gpu_regs[HP] & 0x3ff;
				int vperiod = (gpu_regs[VP] & 0x7ff) + 1;
				int vbend = MAX(gpu_regs[VBE], gpu_regs[VDB]) & 0x1ff;
				int vbstart = MIN(gpu_regs[VBB], gpu_regs[VDE]) & 0x1ff;

				/* adjust for the half-lines */
				if (hperiod != 0 && vperiod != 0 && hbend < hbstart && vbend < vbstart && hbstart < hperiod)
				{
					rectangle visarea;
					visarea.min_x = hbend >> 1;
					visarea.max_x = hbstart >> 1;
					visarea.min_y = vbend >> 1;
					visarea.max_y = vbstart >> 1;
					video_screen_configure(space->machine->primary_screen, hperiod >> 1, vperiod >> 1, &visarea, HZ_TO_ATTOSECONDS((double)COJAG_PIXEL_CLOCK * 2 / hperiod / vperiod));
				}
				break;
			}
		}
