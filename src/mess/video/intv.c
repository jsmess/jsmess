...
VIDEO_START( intv )
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        for (j = 0; j < 16; j++) {
            for (k = 0; k < 128; k++) {
                state->sprite_buffers[i][j][k] = 0;
            }
        }
        state->collision_registers[i] = 0;
    }
    for(i = 0; i < 4; i++) {
        state->color_stack[i] = 0;
    }
    state->color_stack_mode = 0;
    state->color_stack_offset = 0;
    state->stic_handshake = 0;
    state->border_color = 0;
    state->col_delay = 0;
    state->row_delay = 0;
    state->left_edge_inhibit = 0;
    state->top_edge_inhibit = 0;

    state->gramdirty = 1;
    for(i=0;i<64;i++) {
        state->gram[i] = 0;
        state->gramdirtybytes[i] = 1;
    }
#endif
}
...
static int sprites_collide(intv_state *state, int spriteNum0, int spriteNum1) {
    ...
    intv_sprite_type* s1 = &state->sprite[spriteNum0];
    intv_sprite_type* s2 = &state->sprite[spriteNum1];
    ...
    for (x = 0; x < width; x++) {
        for (y = 0; y < height; y++) {
            if (state->sprite_buffers[spriteNum0][x0-x1+x][r0y+y] &&
                    state->sprite_buffers[spriteNum1][x0-x2+x][r1y+y])
                return TRUE;
        }
    }
...
static void determine_sprite_collisions(intv_state *state)
{
    ...
    int i, j;
    for (i = 0; i < 7; i++) {
        intv_sprite_type* s1 = &state->sprite[i];
        if (s1->xpos == 0 || !s1->coll)
            continue;

        for (j = i+1; j < 8; j++) {
            intv_sprite_type* s2 = &state->sprite[j];
    ...
    for (i = 0; i < 8; i++) {
        ...
        for (j = 0; j < spritePixelHeight; j++) {
            ...
            if (s->grom)
                ...
                nextData = state->gram[nextMemoryLocation];
            else
                nextData = 0xFFFF;
            nextX = (s->xflip ? (s->doublex ? 15 : 7) : 0);
            nextY = (s->yflip ? (spritePixelHeight - j - 1) : j);
            xInc = (s->xflip ? -1: 1);
            state->sprite_buffers[i][nextX][nextY] = ((nextData & 0x0080) != 0);
            state->sprite_buffers[i][nextX + xInc][nextY] = (s->doublex
                    ? ((nextData & 0x0080) != 0)
                    : ((nextData & 0x0040) != 0));
            state->sprite_buffers[i][nextX + (2*xInc)][nextY] = (s->doublex
                    ? ((nextData & 0x0040) != 0)
                    : ((nextData & 0x0020) != 0));
            state->sprite_buffers[i][nextX + (3*xInc)][nextY] = (s->doublex
                    ? ((nextData & 0x0040) != 0)
                    : ((nextData & 0x0010) != 0));
            state->sprite_buffers[i][nextX + (4*xInc)][nextY] = (s->doublex
                    ? ((nextData & 0x0020) != 0)
                    : ((nextData & 0x0008) != 0));
            state->sprite_buffers[i][nextX + (5*xInc)][nextY] = (s->doublex
                    ? ((nextData & 0x0020) != 0)
                    : ((nextData & 0x0004) != 0));
            state->sprite_buffers[i][nextX + (6*xInc)][nextY] = (s->doublex
                    ? ((nextData & 0x0010) != 0)
                    : ((nextData & 0x0002) != 0));
            state->sprite_buffers[i][nextX + (7*xInc)][nextY] = (s->doublex
                    ? ((nextData & 0x0010) != 0)
                    : ((nextData & 0x0001) != 0));
            if (!s->doublex)
                continue;

            state->sprite_buffers[i][nextX + (8*xInc)][nextY] =
                    ((nextData & 0x0008) != 0);
            state->sprite_buffers[i][nextX + (9*xInc)][nextY] =
                    ((nextData & 0x0008) != 0);
            state->sprite_buffers[i][nextX + (10*xInc)][nextY] =
                    ((nextData & 0x0004) != 0);
            state->sprite_buffers[i][nextX + (11*xInc)][nextY] =
                    ((nextData & 0x0004) != 0);
            state->sprite_buffers[i][nextX + (12*xInc)][nextY] =
                    ((nextData & 0x0002) != 0);
            state->sprite_buffers[i][nextX + (13*xInc)][nextY] =
                    ((nextData & 0x0002) != 0);
            state->sprite_buffers[i][nextX + (14*xInc)][nextY] =
                    ((nextData & 0x0001) != 0);
            state->sprite_buffers[i][nextX + (15*xInc)][nextY] =
                    ((nextData & 0x0001) != 0);
        }
...
static void render_color_stack_mode(running_machine *machine, bitmap_t *bitmap)
{
    ...
    for (h = 0; h < 240; h++) {
        ...
        if ((nextCard & 0x1800) == 0x1000) {
            UINT8 csColor = state->color_stack[csPtr];
        ...
        //color stack mode
            ...
            bgcolor = state->color_stack[csPtr] & 0x0F;
		...
		for(row = 0; row < 12; row++)
		{
			for(col = 0; col < 20; col++)
			{
				...
				if (p_bit && (!g_bit)) // colored squares mode
				{
					...
					if (colora == 7) colora = state->color_stack[3];
					if (colorb == 7) colorb = state->color_stack[3];
					if (colorc == 7) colorc = state->color_stack[3];
					if (colord == 7) colord = state->color_stack[3];
				...
				else // normal color stack mode
				{
					if (n_bit) // next color
					{
						state->color_stack_offset += 1;
						state->color_stack_offset &= 3;
					}
					...
					bgcolor = state->color_stack[state->color_stack_offset];
					code = (value>>3)&0xff;
 ...
 READ8_HANDLER( intvkbd_tms9927_r )
{
	...
	{
		case 8:
			rv = state->tms9927_cursor_row;
			break;
		case 9:
			/* note: this is 1-based */
			rv = state->tms9927_cursor_col;
			break;
		case 11:
			state->tms9927_last_row = (state->tms9927_last_row + 1) % state->tms9927_num_rows;
			rv = state->tms9927_last_row;
			break;
		default:
			rv = 0;
	}
...
WRITE8_HANDLER( intvkbd_tms9927_w )
{
	...
	{
		case 3:
			state->tms9927_num_rows = (data & 0x3f) + 1;
			break;
		case 6:
			state->tms9927_last_row = data;
			break;
		case 11:
			state->tms9927_last_row = (state->tms9927_last_row + 1) % state->tms9927_num_rows;
			break;
		case 12:
			/* note: this is 1-based */
			state->tms9927_cursor_col = data;
			break;
		case 13:
			state->tms9927_cursor_row = data;
			break;
	}
}
...
//  char c;
	...
	{
		current_row = (state->tms9927_last_row + 1) % state->tms9927_num_rows;
		for(y=0;y<24;y++)
		{
			for(x=0;x<40;x++)
			{
			...
			if (current_row == state->tms9927_cursor_row)
			{
				...
				drawgfx_transpen(bitmap, NULL,
					...
					7,   /* white   */
					0,0,
					(state->tms9927_cursor_col-1)<<3,y<<3, 0);
			}
			current_row = (current_row + 1) % state->tms9927_num_rows;
		}
	}
