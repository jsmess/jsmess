...
static mame_timer *cdp1861_dmao_timer;
...
static void cdp1861_dma_tick(int ref)
{
	if (cdp1861.dmaout)
	{
		...
		mame_timer_adjust(cdp1861_dmao_timer, MAME_TIME_IN_CYCLES(CDP1861_CYCLES_DMAOUT_HIGH, 0), 0, time_zero);
		cdp1861.dmaout = 0;
	...
	else
	{
		...
		mame_timer_adjust(cdp1861_dmao_timer, MAME_TIME_IN_CYCLES(CDP1861_CYCLES_DMAOUT_LOW, 0), 0, time_zero);
		cdp1861.dmaout = 1;
...
void cdp1861_dma_w(UINT8 data)
{
	...
	{
		mame_timer_adjust(cdp1861_dmao_timer, double_to_mame_time(TIME_NEVER), 0, time_zero);
	}
}
...
void cdp1861_sc(int state)
{
	if (state == CDP1802_STATE_CODE_S3_INTERRUPT)
	{
		...
		mame_timer_adjust(cdp1861_dmao_timer, MAME_TIME_IN_CYCLES(CDP1861_CYCLES_INT_DELAY, 0), 0, double_to_mame_time(TIME_NEVER));
	}
}
...
MACHINE_RESET( cdp1861 )
{
	...
	cdp1861_dmao_timer = mame_timer_alloc(cdp1861_dma_tick);
