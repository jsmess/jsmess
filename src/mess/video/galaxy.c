...
static TIMER_CALLBACK( gal_video )
{
	...
	if (galaxy_interrupts_enabled == TRUE)
	{
		...
		if ((gal_cnt >= 48 * 2) && (gal_cnt < 48 * 210))  // display on screen just first 208 lines
		{
			...
			UINT16 addr = (cpu_get_reg(machine->device("maincpu"), Z80_I) << 8) | cpu_get_reg(machine->device("maincpu"), Z80_R) | ((galaxy_latch_value & 0x80) ^ 0x80);
			if (mode == 0)
			{
				// Text mode
				if (first == 0 && (cpu_get_reg(machine->device("maincpu"), Z80_R) & 0x1f) == 0)
				{
			...
			{ // Graphics mode
				if (first < 4 && (cpu_get_reg(machine->device("maincpu"), Z80_R) & 0x1f) == 0)
				{
					// Due to a fact that on real processor latch value is set at
