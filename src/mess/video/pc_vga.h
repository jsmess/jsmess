...
struct pc_vga_interface
{
	/* VGA memory mapper */
	int vga_memory_bank;
	void (*map_vga_memory)(offs_t begin, offs_t end, read8_machine_func rh, write8_machine_func wh);

	/* VGA dipswitch (???) */
	read8_machine_func read_dipswitch;
...
#if 0
        int i;
        UINT8 *memory=memory_region(machine, REGION_CPU1)+0xc0000;
        UINT8 chksum;
