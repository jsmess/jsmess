...
/****************Games*******************************************************/
	...
	DRIVER( supercon )
	DRIVER( diablo68 )  // Diablo 68000
	DRIVER( sfortea )   // Super Forte 'A' (5mhz?)
	DRIVER( sforteb )   // Super Forte 'B' (5mhz?)
	DRIVER( sforteba )  // Super Forte 'B' (6mhz?)
	DRIVER( sfortec )   // Super Forte 'C' (probably 6mhz)

//	DRIVER( sexperta )  // Super Expert 'A' no dumps yet
	DRIVER( sexpertb )  // Super Expert 'B'
	DRIVER( sexpertc ) // Super Expert 'C' // version unknown
	DRIVER( sexper36 ) // Super Expert 'C' V3.6
...
//  DRIVER( mephisto )  /* Mephisto  */
	...
	DRIVER( academy )      /* Academy 65C02    */
	DRIVER( monteciv ) /* Monte Carlo IV */
	DRIVER( megaiv ) /* Mega IV */
	DRIVER( van16 )      /* Vancouver 68000    */
	DRIVER( van32 )      /* Vancouver 68020    */
	DRIVER( alm32 )      /* Almeria 68020    */
	DRIVER( gen32 )      /* Genius030 68030 V4.00   */
	DRIVER( gen32_41 )      /* Genius030 68030 V4.01   */
	DRIVER( gen32_oc )      /* Genius030 68030 V4.01 2x overclocked for max ELO */
	DRIVER( alm16 )      /* Almeria 68000   */
	DRIVER( lyon16 )      /* Lyon 68000    */
	DRIVER( lyon32 )      /* Lyon 68020    */
	DRIVER( bpl32 )   /* Berlin Pro London 68020 */
	DRIVER( berlinp )   /* Berlin Pro 68020 */
	DRIVER( lond030 ) /* London Program 68030 33mhz */
	DRIVER( lond020 ) /* London Program 68020 12mhz */

	/* Saitek */
	DRIVER( risc ) /* Saitek 2500 14MHZ (skeleton) */

	/* TASC Final Chess Card */
	DRIVER ( finalchs )  /* skeleton */
