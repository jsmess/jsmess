...
/*
   The flag for 80-track drives is set in the controller, so we need to
   include it here.
*/
...
static int ti99_get_tracks_per_disk(floppy_image *floppy)
{
	...
	if (ti99_image_in_80_track_drive())
	{
		drivetracks = 83;
	}
	return drivetracks;
...
static floperr_t ti99_sdf_read_track(floppy_image *floppy, int head, int track, UINT64 offset, void *buffer, size_t buflen)
{
	...
	if (ti99_image_in_80_track_drive() && tag->tracks<=40)
	{
		imgtrack = track / 2;
	}
    ...
    */
	...
	while (current_pos < buflen)
	{
		...
		if (new_pos==TI99DSK_BLOCKNOTFOUND)
		{
			...
			logerror("Cannot find any address block for track %d, head %d.\n", track, head);
			return FLOPPY_ERROR_INVALIDIMAGE;
		}
		if (!is_fm)
			...
			if (wtrack == imgtrack && whead == head)
			{
				...
				else
				{
					...
					if (track_image[current_pos]==0xfb)
					{
						...
						logerror("DAM not found. Don't write sector %d\n", sector);
 ...
 *     Sector:             ss       |
 *       Size:             01       |
 *        CRC:         2*  f7       +--- repeat 9 times (8 rarely)
 *       Gap2:        11*  ff       |
 *                     6*  00       |
 *        DAM:             fb       |
 *    Content:       256*  xx       |
 ...
 *     Sector:             ss       |
 *       Size:             01       |
 *        CRC:         2*  f7       +--- repeat 18 times (16 rarely)
 *       Gap2:        22*  4e       |
 *                    12*  00       |
 *       Sync:         3*  a1       |
 *        DAM:             fb       |
...
FLOPPY_OPTIONS_START( ti99 )
	FLOPPY_OPTION( ti99_sdf, "dsk",			"TI99 sector dump (v9t9)",	ti99_sdf_identify,	ti99_sdf_construct,
		HEADS([1]-2)
		TRACKS(35-[40]-80)
		SECTORS(8/9/16/[18]/36)
		SECTOR_LENGTH([256])
		FIRST_SECTOR_ID(0))
	FLOPPY_OPTION( ti99_tdf, "dsk,dtk",			"TI99 track dump (pc99)",	ti99_tdf_identify,	ti99_tdf_construct,
		TRACKS(35-[40]-80)
		SECTORS(8/9/16/[18]/36)
		SECTOR_LENGTH([256])
		FIRST_SECTOR_ID(0))
