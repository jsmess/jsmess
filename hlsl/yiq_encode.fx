...
float4 ps_main(PS_INPUT Input) : COLOR
{
	...
	float3 Texel0 = tex2D(DiffuseSampler, Input.Coord0 - float2(-FscScale * 0.00f, 0.5f) / Scaler).rgb;
	float3 Texel1 = tex2D(DiffuseSampler, Input.Coord1 - float2(-FscScale * 0.25f, 0.5f) / Scaler).rgb;
	float3 Texel2 = tex2D(DiffuseSampler, Input.Coord2 - float2(-FscScale * 0.50f, 0.5f) / Scaler).rgb;
	float3 Texel3 = tex2D(DiffuseSampler, Input.Coord3 - float2(-FscScale * 0.75f, 0.5f) / Scaler).rgb;
