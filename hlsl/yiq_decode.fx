...
VS_OUTPUT vs_main(VS_INPUT Input)
{
	...
	Output.Coord0.zw = Input.TexCoord + float2(1.50f / RawWidth, 0.0f);
	Output.Coord1.zw = Input.TexCoord + float2(1.75f / RawWidth, 0.0f);
	Output.Coord2.zw = Input.TexCoord + float2(2.00f / RawWidth, 0.0f);
	Output.Coord3.zw = Input.TexCoord + float2(2.25f / RawWidth, 0.0f);
	Output.Coord4.zw = Input.TexCoord + float2(2.50f / RawWidth, 0.0f);
	Output.Coord5.zw = Input.TexCoord + float2(2.75f / RawWidth, 0.0f);
...
float4 ps_main(PS_INPUT Input) : COLOR
{
	...
	float2 OrigC2 = tex2D(CompositeSampler, Input.Coord4.xy).rg;
	float4 C = OrigC;
	float2 C2 = OrigC2;
	...
	float2 I2 = (C2 - Y) * sin(W * Tc2);
	float2 Q2 = (C2 - Y) * cos(W * Tc2);
