...
float4 ps_main(PS_INPUT Input) : COLOR
{
	...
	clip((BaseCoord.x < 1.0f / TargetWidth) ? -1 : 1);
	clip((BaseCoord.y < 3.0f / TargetHeight) ? -1 : 1);
	...
	float3 ChromaTexel = lerp(float3(RedChromaTexel, GrnChromaTexel, BluChromaTexel), LumaTexel, ChannelFrac);
	...
	float ShadowCoordX = frac(ShadowCoord.x / ShadowPixelSizeX) * ShadowU + 0.5f / (ShadowWidth * ShadowU);
	float ShadowCoordY = frac(ShadowCoord.y / ShadowPixelSizeY) * ShadowV + 0.5f / (ShadowHeight * ShadowV);
